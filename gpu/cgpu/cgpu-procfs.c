...
static const struct proc_ops cgpu_version_node_fops = {
    .proc_open = cgpu_version_node_open, // 打开操作
    .proc_read = seq_read,               // 读取操作
    .proc_lseek = seq_lseek,             // 偏移操作
    .proc_release = single_release,      // 释放操作
};
...
static const struct proc_ops cgpu_id_node_fops = {
    .proc_open = cgpu_id_node_open, // 打开操作
    .proc_read = seq_read,          // 读取操作
    .proc_lseek = seq_lseek,        // 偏移操作
    .proc_release = single_release, // 释放操作
};
...
static const struct proc_ops cgpu_meminfo_node_fops = {
    .proc_open = cgpu_meminfo_node_open,
    .proc_read = seq_read,
...
static const struct proc_ops cgpu_memsize_node_fops = {
    .proc_open = cgpu_memsize_node_open,   // 打开操作
    .proc_read = seq_read,                 // 读取操作
    .proc_write = cgpu_memsize_node_write, // 写入操作
    .proc_lseek = seq_lseek,               // 偏移操作
    .proc_release = single_release,        // 释放操作
};
...
static ssize_t cgpu_weight_node_write(struct file *filp, const char __user *buf,
...
static const struct proc_ops cgpu_weight_node_fops = {
    .proc_open = cgpu_weight_node_open,   // 打开操作
    .proc_read = seq_read,                // 读取操作
    .proc_write = cgpu_weight_node_write, // 写入操作
    .proc_lseek = seq_lseek,              // 偏移操作
    .proc_release = single_release,       // 释放操作
};
...
static const struct proc_ops cgpu_freeweight_node_fops = {
    .proc_open = cgpu_freeweight_node_open, // 打开操作
    .proc_read = seq_read,                  // 读取操作
    .proc_lseek = seq_lseek,                // 偏移操作
    .proc_release = single_release,         // 释放操作
};
int create_new_node(char *name, int i) {
  ...
  inst = proc_create_data("id", 0444, inst_root, &cgpu_id_node_fops, priv);
  if (!inst) {
    pr_err("failed to create node \"id\"\n");
    goto err_procfs;
  }

  inst = proc_create_data("meminfo", 0444, inst_root, &cgpu_meminfo_node_fops,
                          priv);
  if (!inst) {
  ...
  inst = proc_create_data("memsize", 0666, inst_root, &cgpu_memsize_node_fops,
                          priv);
  if (!inst) {
  ...
  inst =
      proc_create_data("weight", 0666, inst_root, &cgpu_weight_node_fops, priv);
  if (!inst) {
...
static long cgpu_inst_ctl_ioctl(struct file *filp, unsigned int cmd,
                                unsigned long arg) {
  void *argp = (void *)arg;
  void *argi, *argo;
  size_t size = 0;
...
static const struct proc_ops cgpu_inst_ctl_fops = {
    .proc_open = nonseekable_open,     // 打开操作
    .proc_read = cgpu_inst_ctl_read,   // 读取操作
    .proc_write = cgpu_inst_ctl_write, // 写入操作
    .proc_ioctl = cgpu_inst_ctl_ioctl, // ioctl 操作
    .proc_lseek = no_llseek,           // 偏移操作
};
...
static const struct proc_ops cgpu_default_ms_node_fops = {
        .proc_open    = cgpu_default_ms_node_open,   // 打开操作
        .proc_read    = seq_read,                    // 读取操作
        .proc_write   = cgpu_default_ms_node_write,  // 写入操作
        .proc_lseek   = seq_lseek,                   // 偏移操作
        .proc_release = single_release,             // 释放操作
};
...
static const struct proc_ops cgpu_major_node_fops = {
    .proc_open = cgpu_major_node_open, // 打开操作
    .proc_read = seq_read,             // 读取操作
    .proc_lseek = seq_lseek,           // 偏移操作
    .proc_release = single_release,    // 释放操作
};
...
static ssize_t cgpu_policy_node_write(struct file *filp, const char __user *buf,
                                      size_t count, loff_t *ppos) {
  char tmp[3];
  int ret;
...
static const struct proc_ops cgpu_policy_node_fops = {
    .proc_open = cgpu_policy_node_open,   // 打开操作
    .proc_read = seq_read,                // 读取操作
    .proc_write = cgpu_policy_node_write, // 写入操作
    .proc_lseek = seq_lseek,              // 偏移操作
    .proc_release = single_release,       // 释放操作
};
...
static const struct proc_ops cgpu_max_inst_node_fops = {
    .proc_open = cgpu_max_inst_node_open,   // 打开操作
    .proc_read = seq_read,                  // 读取操作
    .proc_write = cgpu_max_inst_node_write, // 写入操作
    .proc_lseek = seq_lseek,                // 偏移操作
    .proc_release = single_release,         // 释放操作
};
...
int cgpu_km_procfs_init(int num) {
  ...
  for (i = 0; i < num; i++) {
    ...
    node_policy = proc_create_data(PROCFS_POLICY_NODE, 0666, proc_insts[i],
                                   &cgpu_policy_node_fops, (void *)data);
    node_max_inst = proc_create_data(PROCFS_MAX_INST_NODE, 0666, proc_insts[i],
                                     &cgpu_max_inst_node_fops, (void *)data);

    node_weight = proc_create_data(PROCFS_FREE_WEIGHT_NODE, 0444, proc_insts[i],
                                   &cgpu_freeweight_node_fops, (void *)data);
  ...
  node_default_memsize =
      proc_create_data(PROCFS_DEF_MEMSIZE_NODE, 0666, proc_root,
                       &cgpu_default_ms_node_fops, NULL);
  ...
  node_major = proc_create_data(PROCFS_MAJOR_NODE, 0444, proc_root,
                                &cgpu_major_node_fops, NULL);
  if (!node_major) {
    pr_err("failed to create procfs node %s\n", PROCFS_MAJOR_NODE);
    ret = -EFAULT;
    goto err;
  ...
  node_version = proc_create_data("version", 0444, proc_root,
                                  &cgpu_version_node_fops, NULL);
  group_num = num;
  return 0;
